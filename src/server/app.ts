...
const roomToClientRoom = (room : Room) : ClientRoom => ({name: room.name, hostName: room.host.name, isPrivate: room.isPrivate, status: room.status, roomId: room.roomId});
...
const getUser = (socketId : string) : User | undefined => users.find(user => user.socketId === socketId);
