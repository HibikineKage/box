import reducer, {MATCH_ROOM_FAILED, JOIN_ROOM_FAILED, MATCH_ROOM_TIMEOUT} from './ducks';
...
describe('matching reducer should return ', () => {
  ...
  test('Timeout when timeouted', () => {
    const timeoutState = reducer(matchingState, {type: MATCH_ROOM_TIMEOUT})
    expect(timeoutState).toEqual({status: MatchingStatus.Timeout})
  })
  test('Default when joining failed.', () => {
    const joiningFailedState = reducer(waitingState, {type: JOIN_ROOM_FAILED});
