...
import { INITIAL_COUNT, State as GameState, gameTick, countDown } from './ducks';
import { State } from '../ducks';
import { IPlayer } from '../player/ducks';

const TPS = 20;
const gameSelector = (state: State) => state.game;
const isGameFinished = (players: IPlayer[]): boolean => players.some(p => p.boxCount == 0);
export function* ticker(initialTick: number) {
  let tickCount = 0;
  while (isGameFinished((yield select(gameSelector)).players)) {
    yield put(gameTick());
    yield delay(initialTick + tickCount * tickCount / TPS - performance.now());
    tickCount += 1;
  }
}
